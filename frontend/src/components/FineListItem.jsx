...
const PenaltyListItem = ({ penalty, paying, onPay }) => {
  return (
    <motion.div
    ...
    >
      <div className="w-10 h-10 rounded-lg bg-foreground/5 flex items-center justify-center flex-shrink-0">
        {penalty.paid ? <CheckCircle2 size={20} className="text-green-500" /> : <AlertCircle size={20} className="text-red-500" />}
      </div>

      <div className="flex-1 min-w-0">
        <p className="font-bold truncate">{penalty.reason}</p>
        <p className="text-sm text-foreground/60 mt-1">
          {new Date(penalty.createdAt).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}
        </p>
        {penalty.paid && (
      ...
      <span className="font-black flex items-center flex-shrink-0"><IndianRupee size={16} />{penalty.amount}</span>
